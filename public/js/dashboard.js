...
async function loadApprovals() {
  try {
    const url = currentCategory ? `/api/approvals?category=${currentCategory}` : '/api/approvals';
...
function renderList(approvals) {
  ...
  emptyEl.style.display = approvals.length === 0 ? 'block' : 'none';
  ...
  approvals.forEach(a => {
    ...
    if (existing) {
      ...
      const meta = existing.querySelector('.comment-count');
      if (meta) meta.textContent = a.comment_count || 0;
      return;
    }
...
function createCard(a) {
  ...
  const extraPages = imageCount > 3 ? imageCount - 3 : 0;
  ...
  card.innerHTML = `
    ...
    <div class="trello-card-thumbs">
      <img src="/uploads/${a.image1}" alt="" loading="lazy">
      <img src="/uploads/${a.image2}" alt="" loading="lazy">
      <img src="/uploads/${a.image3}" alt="" loading="lazy">
    </div>
    <div class="trello-card-footer">
      <div class="trello-card-meta">
        ...
        ${extraPages > 0 ? `
          <span title="${imageCount} páginas en total">
...
function renderDetail(a) {
  ...
  detailBody.innerHTML = `
    ...
    <div class="detail-options">
      ${(a.images || [a.image1, a.image2, a.image3]).map((filename, idx) => {
        const n = idx + 1;
        const label = a.category === 'Diario' ? `Página ${n}` : `Opción ${n}`;
        return `
          <div class="detail-option" data-option="${n}" onclick="selectDetailOption(${n})">
            <div class="detail-option-label">${label}</div>
            ...
            <div class="detail-option-actions">
              ...
              <button class="btn-mark" onclick="event.stopPropagation(); markOption('${filename}', ${n})">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 19l7-7 3 3-7 7-3-3z"/><path d="M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z"/><path d="M2 2l7.586 7.586"/><circle cx="11" cy="11" r="2"/></svg>
                Remarcar
              </button>
            </div>
    ...
    <div class="detail-comments">
      ...
      <div class="comment-form">
        <textarea id="comment-text" placeholder="Escribe un comentario o adjunta una imagen..."></textarea>
  ...
  // listener para preview del nombre del archivo adjunto
...
function renderComments(comments) {
  if (!comments.length) {
    return '<div class="comments-empty">Sin comentarios. Sé el primero en comentar o adjuntar una marca.</div>';
  }
  return comments.map(c => {
    const date = new Date(c.created_at).toLocaleString('es-ES', {
      day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'
    });
    const editedTag = c.updated_at
      ? `<span class="comment-edited" title="Editado el ${new Date(c.updated_at).toLocaleString('es-ES')}"> · editado</span>`
      : '';
    return `
      <div class="comment-item" data-comment-id="${c.id}">
        <div class="comment-header">
          ...
          <div class="comment-actions">
            ${c.content ? `
              <button class="comment-action-btn" onclick="startEditComment(${c.id})" title="Editar">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              </button>
            ` : ''}
            <button class="comment-action-btn danger" onclick="deleteComment(${c.id})" title="Eliminar">
...
window.selectDetailOption = function (n) {
  ...
  document.getElementById('detail-approve').disabled = false;
};

window.markOption = function (imgFilename, optionNumber) {
  openDrawModal(`/uploads/${imgFilename}`, async (blob, commentText) => {
    const fd = new FormData();
    // Use "Página" for Diario, "Opción" for others
    const detailData = detailBody.querySelector('.detail-option-label');
    const isPaged = detailData && detailData.textContent.startsWith('Página');
    const defaultText = isPaged ? `Marca sobre Página ${optionNumber}` : `Marca sobre Opción ${optionNumber}`;
    const finalText = (commentText && commentText.trim()) ? commentText.trim() : defaultText;
    fd.append('content', finalText);
...
window.submitComment = async function () {
  ...
  if (!text && !file) {
    showToast('Escribe un comentario o adjunta una imagen', 'error');
    return;
  }
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && detailModal.classList.contains('active')) {
    // no cerrar si el lightbox o el draw modal están abiertos
...
loadApprovals();
setInterval(loadApprovals, 20000);
